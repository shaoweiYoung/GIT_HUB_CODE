...
# 定义Richard函数
def richard_function(x, A, K, B, M, nu):
    return A + (K / ((1 + np.exp(-B * (x - M))) ** (1/nu)))

# 拟合Richard函数到数据
p0 = [min(y_filtered), max(y_filtered), 1, np.median(x_filtered), 2]  # Initial guess for parameters
popt, pcov = curve_fit(richard_function, x_filtered, y_filtered, p0=p0)

# 计算拟合值
y_fit = richard_function(x_filtered, *popt)
...
# 打印拟合参数和R^2
print(f"Fitted parameters: A={popt[0]}, K={popt[1]}, B={popt[2]}, M={popt[3]}, nu={popt[4]}")
print(f"R^2: {r_squared}")

# 绘制数据点和拟合曲线
plt.scatter(x_filtered, y_filtered, label='Data')
x_fit = np.linspace(min(x_filtered), max(x_filtered), 100)
y_fit_curve = richard_function(x_fit, *popt)
plt.plot(x_fit, y_fit_curve, color='red', label='Fitted Richard function')

# 显示拟合函数表达式
func_text = f"Fitted function:\n$A + \\frac{{K}}{{(1 + e^{{-B(x - M)}})^{{1/nu}}}}$\n"
