...
# 定义一元悬链线类函数
def catenary_function(x, a, b, c):
    return a * np.cosh(x / b) + c

# 拟合一元悬链线类函数到数据
popt, pcov = curve_fit(catenary_function, x_filtered, y_filtered)

# 计算拟合值
y_fit = catenary_function(x_filtered, *popt)
...
y_fit_curve = catenary_function(x_fit, *popt)
plt.plot(x_fit, y_fit_curve, color='red', label='Fitted catenary function')

# 显示拟合函数表达式
func_text = f"Fitted function:\n$y = a \\cdot \\cosh(\\frac{{x}}{{{popt[1]:.3f}}}) + {popt[2]:.3f}$\n"
